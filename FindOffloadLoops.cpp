...
//    
//      opt -load-pass-plugin ./libFindOffloadLoops.so -passes=find-offload-loops `\`
...
class InductionLoopVisitor {
  ...
  public:
    InductionLoopVisitor(SmallVector<const Loop*, 4> &InductionLoop) : InductionLoop(InductionLoop) {};
    bool isDone(){
      return false;
    }
...
class AccessNode {
  ...
  public:
    // this constructor does a lot of the analysis to create the information contained in AccessNode:
    // Regularity of Access
    // Insertion Point for LoopNodeTree
    // Maybe SCEV Expression for multipliers - for now just constant multiplier based on nesting level
    AccessNode(Loop* LoopPtr, LoadInst* LoadPtr, ScalarEvolution *SE, DominatorTree *DT) : 
      LoopPtr(LoopPtr), 
      LoadPtr(LoadPtr), 
      SE(SE), 
      DT(DT) 
    {
      ...
      SCEVTraversal<InductionLoopVisitor> SCEVTraverser = SCEVTraversal<InductionLoopVisitor>(IndLoopV);
      ...
      assert(FirstNonInductionLoop->contains(LoadPtr) && "Load not in FirstNonInductionLoop, something went wrong");
    ...
    Loop* getFirstNonInduction(){
      return FirstNonInductionLoop;
    }
    float getTripcountFactor() {
      return TripcountFactor;
    }
...
struct LoopNode {
  ...
  std::list<std::shared_ptr<LoopNode>> InnerLoops;  // this can be separate from accesses
  std::vector<std::shared_ptr<AccessNode>> AttachedAccesses; // actually just LoadInst for now
  float Regular = 0; 
  float Irregular = 0;
...
void loopNodeDFSPrinter(std::shared_ptr<LoopNode> ThisNode, unsigned Level) {
  errs() << "\n";
  for(unsigned i = 0; i<Level; ++i) errs() << "\t";
  errs() << ThisNode->Name << " Ratio: " << ThisNode->OffloadRatio << " No.ofAttached: " << ThisNode->AttachedAccesses.size();
  ++Level;
...
class FindRanges {
  private:
    ...
    // sums up the OffloadRatios of all LoopNodes pointed to in the interval [Iter1, Iter2]
    // Iter1 and Iter2 can point to the same element
    float sumUp(
      std::list<std::shared_ptr<offload::LoopNode>>::const_iterator Iter1, 
  ...
  FindRanges (const std::list<std::shared_ptr<LoopNode>> &InnerLoopNodes, 
    float Threshold = 0.1 ) 
      : 
    InnerLoopNodes(InnerLoopNodes),
    Threshold(Threshold) 
  {
    for(auto Iter1 = InnerLoopNodes.begin(); Iter1 !=InnerLoopNodes.end(); ++Iter1){
    ...
    if(!TupleList.empty()) {
      assert(((TupleList.back()).end < InnerLoopNodes.size()) && "tuple list doesn't match inner loop nodes!");
      printRanges();
    } else {
  ...
  void printRanges() {
    ...
    for(auto T : TupleList) {
      while(std::distance(InnerLoopNodes.begin(),Iter) < T.begin) { ++Iter;}
      errs() << T.begin << ": ";
      (**Iter).LoopPtr->print(errs());
      errs() << " \n";
      while(std::distance(InnerLoopNodes.begin(),Iter) < T.end) { ++Iter;}
      errs() << T.end << ": ";
      (**Iter).LoopPtr->print(errs());
      errs() << " \n";
      errs() << "Average: " << T.avg << " \n";
  ...
  void getRangesAsList(
    std::list<std::shared_ptr<LoopNode>> *Worklist, 
...
class LoopTreeGen {
  private:
    ...
    void reduceLoadPtr(std::list<std::shared_ptr<AccessNode>> *NodeWorkList){
      ...
      while(Reduce){
        Reduce = false;
        for(auto Iter1 = NodeWorkList->begin(); Iter1 != NodeWorkList->end(); ++Iter1) {
    ...
    void populateLoopTree(){
      ...
      // go over list of LoopTreeNodes and insert them into their Parents' InnerLoop lists
      for(auto RevIter = NewLoopNodes.rbegin(); RevIter != NewLoopNodes.rend(); RevIter++) {
        auto Node = *RevIter;
    ...
    void insertLoadsIntoLoopTree(SmallPtrSet<llvm::LoadInst*,5 > InitialSet){
      ...
      for(auto rvIter = Loops.rbegin(); rvIter != Loops.rend(); ++rvIter) {
        auto L = (*rvIter);
        // go over all loads in TLL and add the ones contained in L to working set, 
        // then remove them from inital set
        ...
        for(auto LoadPtr : WorkingSet) {
          ...
          NewNodePtr =  std::shared_ptr<AccessNode>(new AccessNode(L, LoadPtr, SE, &DT));
    ...
    void calcRegularity(){
      for(auto RevIter = LoopTreeVector.rbegin(); RevIter != LoopTreeVector.rend(); RevIter++) {
        auto Node = *RevIter;
        ...
        if(std::isnan(Node->OffloadRatio)) Node->OffloadRatio = 0.0;
      }
    }
  public:
    // constructor
...
bool runOnFunction(Function &F, FunctionAnalysisManager &AM) {
  ...
  // The pointers describe the first and last loop in a given nesting level to be offloaded, 
  // the two pointers can point to the same loop, 
  // they have to point to loops that are on the same nesting level
  // (meaning they are both nested in the same outer loop, or are both TLLs)
  // one nesting level can have multiple ranges
  ...
  while(!WorkList.empty()){
    ...
    WorkList.pop_front();
    FindRanges RangeFinder = FindRanges(TraversePtr->InnerLoops, OffloadThreshold);
    RangeFinder.getRangesAsList(&WorkList, &OffloadRanges);
  }
  
  assert(((OffloadRanges.size() % 2) == 0) && "Offload Ranges not made up out of pairs");
  errs() << "\n------------------------------------------------------------------------------- \n\n";
  errs() << "There are " <<  (OffloadRanges.size() / 2) << " offload ranges in " << F.getName() << "\n\n";

  for(auto Iter = OffloadRanges.begin(); Iter != OffloadRanges.end(); ++Iter) {
    Loop* Begin = *Iter;
    ++Iter;
    if(Iter == OffloadRanges.end()) break;
    Loop* End = *Iter;
